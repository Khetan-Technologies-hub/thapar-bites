...
- **Flow:** `/draft-brief` writes it → a Manager runs **`/read-brief <#>`** to absorb it and agree a build
  plan → **`/draft-ticket`** drafts against it and stamps **`Brief: #<n>`** into the ticket → **`/review-ticket`**
  checks the ticket delivers the brief.

A brief is **product intent, not implementation** — the technical "how" is decided by the Manager during
